<template>
  <li
    ...
    :class="[
      ...
      `vs-navbar-item-${getActiveTextColor}`
    ]"
  ...
  >
    <slot></slot>
    <span :style="[styleAfter]" class="vs-navbar-after"></span>
  </li>
</template>
<script>
  ...
  computed: {
    ...
    styleHover() {
      if (this.isActiveItem) {
        ...
        return {
          color: this.hover
            ? _color.getColor(this.getActiveTextColor)
            : _color.getColor(this.$parent.$props.textColor)
        }
      }
    }
  },
...
@import '@/assets/style/components/vsNavbar.scss';
</style>
